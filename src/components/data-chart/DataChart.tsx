...
const DataChart = () => {
  ...
  releaseTime.sort();
  const nearTimeIndex =
    releaseTime.findIndex((time) => time === currentTime) - 1;
  const location = {
    X: 60,
    Y: 127,
  };
    ...
    params: {
      ...
      base_date:
        nearTimeIndex > 0
          ...
          : dayjs().clone().subtract(1, "days").format("YYYYMMDD"),
  ...
  const { isLoading, data } = useQuery("getWeather", () =>
    getCurrentWeather(config),
  );

  return <LoadingCard loading={isLoading} data={data} />;
};
